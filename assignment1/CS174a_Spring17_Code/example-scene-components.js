...
Declare_Any_Class( "Bee_Scene",  // An example of drawing a hierarchical object using a "model_transform" matrix and post-multiplication.
  ...
  { 
   'construct'( context )
      { var shapes = { "box" : new        Cube(), 
        ...
        this.define_data_members( { yellow_clay: context.shaders_in_use["Phong_Model"].material( Color(  1,  1, .3, 1 ), .2, 1, .7, 40 ),
                                  ...
                                  purplePlastic: context.shaders_in_use["Phong_Model" ].material( Color( .9,.5,.9, 1 ), .4, .4, .8, 40 ),
                                    ...
                                    lightBlue  : context.shaders_in_use["Phong_Model" ].material( Color( 1,.8,.7, 1 ), .4, .8, .4, 20 ),
                                    lightYellow  : context.shaders_in_use["Phong_Model" ].material( Color( 1,.9,.7, 1 ), .4, .8, .4, 20 ),

                                    lightPink  : context.shaders_in_use["Phong_Model" ].material( Color( .8,.6,.7, 1 ), .4, .8, .4, 20 ),
                                     stars: context.shaders_in_use["Phong_Model"  ].material( Color( .8,.4,.3,1 ), .5, .5, .5, 40, context.textures_in_use["stars.png"] )
        
                                   } );
      },

      'draw_bee_head'(model_transform, graphics_state, head_radius)
      {
        model_transform = mult(model_transform, scale(head_radius, head_radius, head_radius));
        this.shapes.ball       .draw( graphics_state, model_transform,this.lightYellow );
        return model_transform;
      },
      'draw_bee_body'(model_transform, graphics_state, body_height, body_length, body_width, head_radius)
      {
        var x = - head_radius - body_width/2;
        model_transform = mult( model_transform, translation( x, 0, 0 ) );
        model_transform = mult(model_transform, scale(body_width/2, body_length/2, body_height/2));
        this.shapes.box       .draw( graphics_state, model_transform,this.lightPink );
        return model_transform;
      },
      'draw_bee_tail'(model_transform, graphics_state, tail_width, body_length, tail_height)
      {
        var x = - body_length/2 - tail_width/2;
        model_transform = mult( model_transform, translation( x, 0, 0 ) );
        model_transform = mult(model_transform, scale(tail_width/2, tail_height/2, tail_height/2));
      ...
      'draw_leg'(model_transform, graphics_state, is_left)
      {
        var leg_length = 1,
            leg_width = 0.11,
            leg_height = 0.05,
            ...
            leg = model_transform,
            t = graphics_state.animation_time/300;
            var leg = mult(model_transform, rotation(35 * Math.abs(Math.sin(t)), [is_left,0,0]));
            leg_rotation_axis = mult(leg, translation(0, 0,0));
            leg = mult(leg, translation(0, -leg_length, -leg_height));
            leg = mult(leg, scale(leg_width, leg_length, leg_height));
            this.shapes.box.draw(graphics_state, leg, this.lightBlue);
            leg = mult(leg, scale(1/leg_width, 1/leg_length, 1/leg_height));
      ...
      'draw_bee'( model_transform, graphics_state)
      {
        ...
        body_width = 4,
        body_length = 2;
        bee = mult(model_transform, translation(-6,0,0));
        var head = this.draw_bee_head(bee, graphics_state, head_radius);
        head = mult(head, rotation(-90, 0,1,0));
        ...
        var increment = body_width/6;
        var begin = mult(body, translation(body_width/7, - body_length/2 , body_height/2));
        var is_left = 1;
        var is_right = -1;
        var right_begin = mult(begin, translation(0.3,0,- body_width/2));
        //left
        for(var i = 0; i < 3; i++){
          bee = this.draw_leg(begin, graphics_state,is_left);
          bee = mult(bee, translation(0,-2,0));
          bee = this.draw_leg(bee, graphics_state,is_left);
          begin = mult(begin, translation( - increment,0, 0));
        }
        //right
        for(var i = 0; i < 3; i++){
          bee = this.draw_leg(right_begin, graphics_state,is_right);
          bee = mult(bee, translation(0,-2,0));
          bee = this.draw_leg(bee, graphics_state,is_right);
          right_begin = mult(right_begin, translation( - increment,0, 0));
        }
        return body;
      },
      ...
      { 
        ...
        for(var i = 0; i < 8; i++){
          model_transform = this.draw_tree_truck(model_transform,graphics_state);
          model_transform = mult(model_transform, translation(0,2,0));
        }
      ...
      'draw_tree_truck'(model_transform, graphics_state)
      {
        var segment_height = 1,
            segment_length = 0.2,
            segment_width = 0.2;
            model_transform = mult(model_transform, rotation(5 * Math.sin(t), [0,0,1]));
            model_transform = mult(model_transform, translation(0, segment_height,segment_width));
            model_transform = mult(model_transform, scale(segment_length, segment_height, segment_width));
            this.shapes.box.draw(graphics_state,model_transform, this.brown_clay);
            model_transform = mult(model_transform, scale(1/segment_length, 1/segment_height, 1/segment_width));
            model_transform = mult(model_transform, translation(0, -segment_height,-segment_width));
            return model_transform;
      },
      'draw_grass'(graphics_state)
      {
        var model_transform = identity(); 
        model_transform = mult(model_transform, translation(0,-8,0));
        model_transform = mult(model_transform, translation(0,1,0));
        model_transform = mult(model_transform, scale(30, 1, 30));
    ...
    'display'( graphics_state )
      { 
        ...
        graphics_state.lights = [ new Light( vec4(  30,  30,  34, 1 ), Color( 0, .4, 0, 1 ), 100000 ),      // Arguments to construct a Light(): Light source position or 
                                  new Light( vec4( -10, -20, -14, 0 ), Color( 1, 1, .3, 1 ), 100    ) ];    // vector (homogeneous coordinates), color, and size.  
       var t = graphics_state.animation_time/100; 
       var bee = model_transform;
       bee = mult(bee, rotation(3 * t, 0, 1, 0));
       bee = mult(bee, translation(0, Math.sin(t), 0));
       bee = this.draw_bee(mult(bee, scale(0.5,0.5,0.5)), graphics_state); 
       bee = mult(bee, translation(0, Math.sin(t), 0));
       this.draw_tree(graphics_state);
       this.draw_grass(graphics_state);
