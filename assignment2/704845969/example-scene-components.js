  ...
  { 'construct'( context )

      {
       ...
       var shapes = { 'triangle'        : new Triangle(),                               // At the beginning of our program, instantiate all shapes we plan to use,
                       ...
                       'desk'            : new Shape_From_File( "kids_desk.obj" )
                     };
    ...
    'display'( graphics_state )
      { var model_transform = identity();             // We have to reset model_transform every frame.
        ...
        **********************************/                                     // From here on down it's just some example shapes drawn 
        ...
        this.shapes.desk.draw( graphics_state,model_transform,this.greyPlastic);
...
var isStart = 0; // used to determine whether user want to start directly
var eat = 1; // variable to determine whether user is in eat mode
var counter = 0; // variable to determine whether 
var grab = 0; //variable to determine whether user grabs food 
var beat = 0; // variable to determine whether user will be punished by professor
//var play = 0;  // 
var r = 1 + counter/5;
var playrate = 1 * r; 
...
var professorPause = 0;
var cameraOn = 0;
var swingRate = 3 * playrate; // variable to determine the swing rate of user's arm
var x = -10, y = 2, z = -22;
var x_v = 0;
var u_x = 0;
var u_y = 0;
  ...
  { 'construct'( context )
      { 
        ...
        var shapes = { "box" : new        Cube(), 
                       ...
                       "desk": new Shape_From_File( "kids_desk.obj" ) ,
                       "strip"          : new Square(),
                       "professor": new Shape_From_File("carl.obj"),
                       "pizza": new Shape_From_File("pizza.obj"),
                       "frenchfries": new Shape_From_File("frenchfries.obj"),
                       "poly": new mypoly(),
                       "usr_hand": new Shape_From_File("master.obj"),
                       "d_donut": new Shape_From_File("donut.obj"),
                       "b_burger": new Shape_From_File("burger.obj"),

                      'debug_text': new Text_Line( 1000 ),
                      'debug_text2': new Text_Line( 1000 ),
        ...
        this.define_data_members( { 
          ...
          yellow_clay: context.shaders_in_use["Phong_Model"].material( Color(  1,  1, .3, 1 ), .2, 1, .7, 40 ),
                                     ...
                                     greyPlastic  : context.shaders_in_use["Phong_Model" ].material( Color( 220/255,220/255,220/255, 1 ), .5, .5, .4, 80 ),
                                     lightPink  : context.shaders_in_use["Phong_Model" ].material( Color( .7,.7,.5, 1 ), .4, .8, .4, 20 ),
                                     white  : context.shaders_in_use["Phong_Model" ].material( Color( 1,1,1, 1 ), .8, .8, .8, 20 ),
                                     blue: context.shaders_in_use["Phong_Model"].material( Color(  0,  0, .5, 1 ), .2, 1, .7, 40 ),
                                     red: context.shaders_in_use["Phong_Model"].material( Color(  0.6,  0, 0.1, 1 ), .2, 1, .7, 40 ),

                                     skin:context.shaders_in_use["Phong_Model"].material( Color(  234/255,  192/255, 134/255, 1 ), .2, 1, .7, 40 ),

                                     p1: context.shaders_in_use["Phong_Model"  ].material( Color( 0,0,0,1 ), .4, .8, .4, 40, context.textures_in_use["pixel1.jpg"] ),
                                     p2: context.shaders_in_use["Phong_Model"  ].material( Color( 1,1,1,1 ), .4, .8, .4, 40, context.textures_in_use["pixel2.jpg"] ),


                                     sinpic: context.shaders_in_use["Phong_Model"  ].material( Color( .5,.5,.5,1 ), .4, .8, .4, 40, context.textures_in_use["sin.png"] ),

                                     deskpic: context.shaders_in_use["Phong_Model"  ].material( Color( 0,0,0,1 ), .8, .8, .8, 40, context.textures_in_use["desk.jpg"] ),
                                     xypic: context.shaders_in_use["Phong_Model"  ].material( Color( .5,.5,.5,1 ), .4, .8, .4, 40, context.textures_in_use["xy.png"] ),
                                     emcpic: context.shaders_in_use["Phong_Model"  ].material( Color( .5,.5,.5,1 ), .4, .8, .4, 40, context.textures_in_use["emc2.png"] ),
                                     woodpic: context.shaders_in_use["Phong_Model"  ].material( Color( .5,.5,.5,1 ), .4, .8, .4, 40, context.textures_in_use["woodCopy.png"] ),
                                     brickpic: context.shaders_in_use["Phong_Model"  ].material( Color( 0.5,0.5,0.5,1 ), .4, .8, .4, 40, context.textures_in_use["brick.jpg"] ),
                                     brickpic2: context.shaders_in_use["Phong_Model"  ].material( Color( 0.1,0.1,0.1,1 ), .4, .8, .4, 40, context.textures_in_use["brick.jpg"] ),
                                     winpic: context.shaders_in_use["Phong_Model"  ].material( Color( 0.2,0.2,0.2,1 ), .4, .4, .4, 80, context.textures_in_use["win.png"] ),
                                     failpic: context.shaders_in_use["Phong_Model"  ].material( Color( 0.2,0.2,0.2,1 ), .4, .4, .4, 80, context.textures_in_use["file.jpg"] ),
                                     sketchpic: context.shaders_in_use["Phong_Model"  ].material( Color( 0.2,0.2,0.2,1 ), .4, .4, .4, 80, context.textures_in_use["sketches.png"] ),
                                     monpic: context.shaders_in_use["Phong_Model"  ].material( Color( 0.2,0.2,0.2,1 ), .4, .4, .4, 80, context.textures_in_use["green.jpg"] ),

                                     prof: context.shaders_in_use["Phong_Model"  ].material( Color( 0,0,0,1 ), .8, .8, .8, 90, context.textures_in_use["professor.png"] ),
                                     pizzacolor: context.shaders_in_use["Phong_Model"  ].material( Color( .5,.5,.5,1 ), .4, .8, .4, 40, context.textures_in_use["pizzaFood.png"] ),
                                     frenchfriesfood: context.shaders_in_use["Phong_Model"  ].material( Color( .5,.5,.5,1 ), .4, .8, .4, 40, context.textures_in_use["frenchfriesfood.png"] ),

                                     donutColor: context.shaders_in_use["Phong_Model"  ].material( Color( .5,.5,.5,1 ), .4, .8, .4, 40, context.textures_in_use["donutcolor.png"] ),
                                     burger_c: context.shaders_in_use["Phong_Model"  ].material( Color( .5,.5,.5,1 ), .4, .8, .4, 40, context.textures_in_use["burgercolor.png"] ),
...

      {
        ...
        controls.add( "1",    this, function() { this.startplay();} );  
        controls.add( "g", this, function() { eat = 1; } );
        controls.add( "p", this, function() { 
                                              eat = 0; 
                                              playrate = 1;
                                            } );
        controls.add( "9", this, function() { beat = 1; } );
        controls.add( "9", this, function() { beat = 0; } , {'type':'keyup'});
      ...
      'background'(model_transform,graphics_state)
      {
        model_transform = mult(model_transform, scale(70, 70, 70));

        this.shapes.box.draw(graphics_state, model_transform, this.brickpic);
        model_transform = mult(model_transform, scale(1/70, 1/70, 1/70));
        return model_transform;
      },

      'update_strings'( user_interface_string_manager)   // Strings that this Scene_Component contributes to the UI:
      { 
        user_interface_string_manager.string_map["tick"]              = "Frame: " + this.tick++;
        user_interface_string_manager.string_map["otherFPS"] = "FPS:" + (1/(this.globals.graphics_state.animation_delta_time / 1000))
      },

      'draw_classroom'(graphics_state)
      {
          this.desk_row_placement(graphics_state);
          var desk_model = this.user_desk(graphics_state);
          var model_transform = identity();
          this.blackborad(graphics_state);

          model_transform = mult(model_transform, translation(0,0,-150));
          model_transform = this.background(model_transform, graphics_state);

      },
      'desk_row_placement' (graphics_state)
      {
        var model_transform = identity();
         graphics_state.lights = [ new Light( vec4(  30,  30,  34, 1 ), Color( 0, .4, 0, 1 ), 100000 ),      // Arguments to construct a Light(): Light source position or 
                                  new Light( vec4( -10, -20, -14, 0 ), Color( 1, 1, .3, 1 ), 100    ) ];    // vector (homogeneous coordinates), color, and size. 
        model_transform = mult(model_transform, scale(1.5, 1.5, 1.5));
        model_transform = mult(model_transform, translation(-7,-2.7,-5));
        this.shapes.desk.draw( graphics_state,model_transform,this.deskpic);
        ...
        model_transform = mult(model_transform, scale(40,40,0.03));
        this.shapes.box.draw( graphics_state,model_transform,this.brickpic2);
      },
      'draw_user'(graphics_state, model_transform)
      {
        var t = graphics_state.animation_time/1000;
        model_transform = mult(model_transform, translation(-3,0,-5));
        this.draw_energy_Bar(model_transform,graphics_state); 
      },
      'draw_arm'(model_transform,graphics_state)
      {
        if(win == 0){
          model_transform = mult(model_transform, translation(2.3,-2.5,17));
          model_transform = mult(model_transform, rotation(-70,0,0,1));
          model_transform = mult(model_transform, rotation(-20,1,0,0));
          this.draw_arm_upper_helper(model_transform, graphics_state);
          this.draw_arm_lower_helper(model_transform,graphics_state);
        }
      },
      'draw_arm_upper_helper'(model_transform, graphics_state)
      {
        var arm_length = 1.9,
            arm_width = 0.23,
            arm_height = 0.4,
            arm = model_transform;
            arm = mult(arm, rotation(110,1,0,0));
            arm = mult(arm, translation( 0,  1.1, 0.2));
            arm = mult(arm, scale(arm_width, arm_length, arm_height));
            this.shapes.ball.draw(graphics_state, arm, this.skin);
            arm = mult(arm, scale(1/arm_width, 1/arm_length, 1/arm_height));
            arm = mult(arm, translation(0, arm_length, arm_height));
      },
      
      'draw_arm_lower_helper'(model_transform, graphics_state)
      {
        var arm_length = 0.8,
            arm_width = 0.2,
            arm_height = 0.18,
            ...
            model_transform = mult(model_transform, translation(-0.2, -0.3, -0.15));
            var arm = model_transform;
            if(eat == 1)
              arm = mult(model_transform, rotation(100 * Math.abs(Math.sin(swingRate * t )), [1,0,0]));
            else 
              arm = mult(model_transform, rotation(100 , [1,0,0]));

              arm = mult(arm, translation(0, -arm_length, -arm_height));
              arm = mult(arm, scale(arm_width, arm_length, arm_height));
              this.shapes.ball.draw(graphics_state, arm, this.skin);

              arm = mult(arm, translation(0, -3*arm_length, -2*arm_height));
              this.draw_hand(arm, graphics_state);

              return arm;
      },
      'draw_hand'(model_transform, graphics_state)
      {
            var ad = mult(model_transform, scale(0.5, 0.5, 0.5));
            ad = mult(ad, translation(-1.9, 0.4, 0.5));
            model_transform = mult(model_transform, scale(0.5, 0.9, 0.9));
            model_transform = mult(model_transform, translation(-1.9, 0.4, 0.5));
            model_transform = mult(model_transform, rotation(-40,0, 1, 0));
            model_transform = mult(model_transform, rotation(-70,1, 1, 1));
            this.shapes.usr_hand.draw(graphics_state, model_transform, this.skin);
            if(grab == 1)
            {
            model_transform = mult(model_transform, translation(1.9, -0.4,- 0.5));
                model_transform = mult(model_transform, rotation(40,0, 1, 0));
                this.shapes.pizza.draw(graphics_state,model_transform, this.pizzacolor);
            }

      },
      'user_desk'(graphics_state)
      {
        var model_transform = identity();
        var t = graphics_state.animation_time/1000,   tilt_spin   = rotation( 70*t, [          .5,          .8,             .1 ] ),
                                                      funny_orbit = rotation(  90*t, [ Math.cos(t), Math.sin(t), .7*Math.cos(t) ] ),
                                                      funny_orbit2 = rotation(  90*t, [ .6*Math.sin(t), Math.cos(t), Math.cos(t) ] ),
                                                      funny_orbit3 = rotation(  30*t, [ Math.cos(t), .5*Math.sin(t), .7*Math.cos(t) ] );
        u_x = 0;
        u_y = -3.5;
        u_z = 15;
        if(win == 0 && lose == 0){
        model_transform = mult(model_transform, translation(u_x,u_y,u_z));
        var pizzaModel = identity();
        pizzaModel= mult(pizzaModel, translation(0,-2.7,15));
        pizzaModel = mult(pizzaModel, scale(0.3,0.3,0.3));
        pizzaModel = mult(pizzaModel, translation(0, 0.1 * Math.sin(t),0));
        var frenchfries = pizzaModel;
        this.shapes.pizza    .draw( graphics_state, mult( pizzaModel, funny_orbit ), this.pizzacolor);
        frenchfries = mult(frenchfries, translation(2,1.5,0));
        this.shapes.frenchfries    .draw( graphics_state, mult( frenchfries, tilt_spin ), this.frenchfriesfood);
        var donutModel = pizzaModel;
        donutModel = mult(donutModel, translation(-3,0.3,0));
        this.shapes.d_donut    .draw( graphics_state, mult( donutModel, funny_orbit3 ), this.donutColor);
        var burgerModel = donutModel;
        burgerModel = mult(burgerModel, translation(-3,0,-1));
        this.shapes.b_burger    .draw( graphics_state, mult( burgerModel, funny_orbit2 ), this.burger_c);
        model_transform = mult(model_transform, rotation( 105, 1, 0, 0 ));
        model_transform = mult(model_transform, scale(5,2,0.03));
        this.shapes.box.draw( graphics_state,model_transform,this.deskpic);
      }
        return model_transform;
      },
      'blackborad'(graphics_state)
      {
        var model_transform = identity();
        model_transform = mult(model_transform, translation(0,15,-30));
        var onboard = model_transform;

        model_transform = mult(model_transform, translation(0,0, -10));
        model_transform = mult(model_transform, scale(20,10,0.03));
        this.shapes.box.draw(graphics_state, model_transform, this.white);
        this.thingsOnBoard(graphics_state,onboard);
      },
      'thingsOnBoard'(graphics_state, model_transform)
      {
        model_transform = mult(model_transform, scale(2,2,0));
        model_transform = mult(model_transform, translation(-6,0,-4));
        this.shapes.box.draw(graphics_state, model_transform, this.emcpic);
        model_transform = mult(model_transform, translation(4,0,0));
        this.shapes.box.draw(graphics_state, model_transform, this.xypic);
        model_transform = mult(model_transform, translation(4,0,0));
        this.shapes.box.draw(graphics_state, model_transform, this.sinpic);
      },

      'draw_professor'(graphics_state)
      {
        var t = graphics_state.animation_time/1000;
        var model_transform = identity();
        model_transform = mult(model_transform, translation(x, y, z));
        model_transform = mult(model_transform, rotation(-10, 0, 1, 0 ));
          if(t < 24 && t > 22) {

            x_v = 30 * Math.sin((t)/5);
            model_transform = mult(model_transform, translation(0, 0, x_v));
          }
          if(t > 24 && t < 28){
            x_v = 30 * Math.sin((t - 6)/5);
             graphics_state.camera_transform = lookAt( [u_x + x_v, u_y + 1.3, u_z],[x + x_v, y, z], [0,1,0]);
             if(grab == 1 || lose == 1){
             graphics_state.camera_transform = lookAt( [0, -1.5, 23.5],[0, 0, -20], [0,1,0]);
                this.check_fail(graphics_state);
                lose = 1;
             }
          } else {
             graphics_state.camera_transform = lookAt( [0, -1.5, 23.5],[0, 0, -20], [0,1,0]);
          }
          model_transform = mult(model_transform, translation(x_v, 0, 0));
          model_transform = mult(model_transform, scale(4,4,4));
          this.shapes.professor.draw(graphics_state, model_transform, this.prof);
      }, 
      'check_win'(graphics_state)
      {
        var model_transform = identity();
        model_transform = mult(model_transform,translation(0,0,12));
        var board = mult(model_transform, scale(8,10,0));
        this.shapes.box.draw(graphics_state, board, this.winpic);
      },
      'check_fail'(graphics_state)
      {
        var model_transform = identity();
        var model_transform = mult(model_transform,translation(0,0,9));
        var board = mult(model_transform, scale(10,6,0));
        this.shapes.box.draw(graphics_state, board, this.failpic);
      },
      'draw_opening'(graphics_state, temp)
      {
        var t = graphics_state.animation_time/1000,   tilt_spin   = rotation( 700*t, [          .1,          .8,             .1 ] ),
                                                      funny_orbit = rotation(  90*t, [ Math.cos(t), Math.sin(t), .7*Math.cos(t) ] );
        var model_transform = identity();
        temp.playbackRate = 1.1;
        model_transform = mult(model_transform, scale(13,13,2));
        if(t < 8){
            this.shapes.box    .draw( graphics_state, mult( model_transform, funny_orbit ), this.p1 );
            temp.play(); 
          }
        if(t > 8 && t < 20 ){
          ...
          model_transform   = mult( translation( -.3, .2, 0 ), font_scale );
          
          this.  shapes.debug_text.set_string( "Press 1 to start!" );
          this.  shapes.debug_text.draw( this.graphics_state, model_transform, this.text_material );
          model_transform   = mult( translation( 0, .2, 0 ), font_scale );
          if(t > 5){
            temp.playbackRate = 1.1;
          }
        }
      },
      'draw_my_own_poly'(model_transform, graphics_state)
      {
        var t = graphics_state.animation_time/1000;
        model_transform = mult(model_transform, scale(0.5,0.5,.5));
        model_transform = mult(model_transform, rotation(20,0,1,0));
        model_transform = mult(model_transform, rotation( 20 * counter * Math.sin(t) ,1,0,0));
        this.shapes.poly.draw(graphics_state, model_transform, this.monpic);
      },
      'draw_energy_Bar'(model_transform,graphics_state)
      {
       if(lose == 0){
          model_transform = mult(model_transform, translation(-2,-2,0));
          var p = model_transform;
          model_transform = mult(model_transform, scale(0.3,1,0.01));
          var myR = counter / 12;
          model_transform = mult(model_transform, translation(0,-1,0.2));
          
          if(counter <= 80 )
          {
            model_transform = mult(model_transform, scale(1, myR,1));  // if myR = counter / 2, need 4 times to go up
            this.shapes.box.draw(graphics_state, model_transform, this.red);
          }
          p = mult(p, translation(0, 3, 0));
          this.draw_my_own_poly(p, graphics_state);
        }
      },
    'display'( graphics_state )
      {  
        var t = graphics_state.animation_time/1000;
        var model_transform = identity(); 
        var temp1 = document.getElementById("audio2");
        var tempMusic = document.getElementById("audio1");
        var ctr = 0;
          if(counter >= 0.8 ){
            counter = counter - 0.1; // set for difficult
          }
          tempMusic.playbackRate = counter/70 + 1; 
          if(win == 1){
            this.check_win(graphics_state);
            var temp0 = document.getElementById("audio3");
            temp0.play();
          }
          else if(lose == 1){
            this.check_fail(graphics_state);
          ...
          if(t < 20 && isStart == 0){
            this.draw_opening(graphics_state,temp1);
          }else{
            if(win == 0 && lose == 0){
              tempMusic.play();
            }
            temp1.pause();
            this.draw_classroom(graphics_state);
            var usr = mult(model_transform, scale(2,2,2));
            usr = mult(usr,translation(0,0,7));
            this.draw_user(graphics_state,usr); 
            this.draw_arm(model_transform, graphics_state);
            this.draw_professor(graphics_state);
            if(counter > 70 && t < 120){
              win = 1;
              tempMusic.pause();
            } 
